...
```kotlin
//新建类DbHelper继承SQLiteOpenHelper接口，实现方法，获取实例，读写数据
val helper = DbHelper(this, "db1", 1)
val db = helper.writableDatabase
db. beginTransaction();
try {
    val cv = contentValuesOf("name1" to "value1","name2" to "value2")
    ...
    db.delete("table1", "name = ?", args)
    db. setTransactionSuccessful();
} finally {
    db. endTransaction();
}
//query可不用beginTransaction
//db.query(...)
```
