...
1. 
   ...
   **消息复用**：Message 对象池减少 GC

2. 

   **线程隔离**：ThreadLocal 保证线程安全

3. 

   **时间排序**：MessageQueue 按 when 排序

4. 

   **异步唤醒**：Native 层 epoll 实现高效等待

5. 

   **同步屏障**：优先处理异步消息，优化 UI 绘制。作用：临时阻塞普通的同步消息，优先处理异步消息。在Android中，同步屏障主要用于优化UI绘制，确保绘制消息（异步消息）能够优先得到处理。

6. 

   **空闲处理**：IdleHandler 利用空闲时间执行非紧急任务
